...
//> Garbage Collection concatenate-peek
  const char* bChars = getStringChars(peek(0));
  int bLength = getStringLength(peek(0));
  const char* aChars = getStringChars(peek(1));
  int aLength = getStringLength(peek(1));
//< Garbage Collection concatenate-peek

  int length = aLength + bLength;
  Value result;
  if (length <= SHORT_STRING_MAX) {
    char buf[SHORT_STRING_MAX + 1];
    memcpy(buf, aChars, aLength);
    memcpy(buf + aLength, bChars, bLength);
    buf[length] = '\0';
    result = copyStringValue(buf, length);
  } else {
...
//> Strings add-strings
      case OP_ADD: {
        if (isAnyString(peek(0)) && isAnyString(peek(1))) {
          concatenate();
