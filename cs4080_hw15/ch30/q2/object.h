...
typedef struct {
  Obj obj;
  int length;
  char* chars;        // points to inlineChars; identical layout to ObjString
  uint32_t hash;
  char inlineChars[SHORT_STRING_MAX + 1];
} ObjShortString;
...
static inline bool isAnyString(Value value) {
  return IS_STRING(value) || IS_SHORT_STRING(value);
}

static inline const char* getStringChars(Value value) {
  return AS_STRING(value)->chars;
}

static inline int getStringLength(Value value) {
