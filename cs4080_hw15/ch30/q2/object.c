...
static ObjShortString* allocateShortString(const char* chars,
                                           int length,
                                           uint32_t hash) {
  ObjShortString* string = ALLOCATE_OBJ(ObjShortString, OBJ_SHORT_STRING);
  string->length = length;
  memcpy(string->inlineChars, chars, length);
  string->inlineChars[length] = '\0';
...
Value takeStringValue(char* chars, int length) {
  if (length <= SHORT_STRING_MAX) {
    ...
    ObjShortString* string = allocateShortString(chars, length, hash);
    FREE_ARRAY(char, chars, length + 1);
...
Value copyStringValue(const char* chars, int length) {
  if (length <= SHORT_STRING_MAX) {
    ...
    return OBJ_VAL(allocateShortString(chars, length, hash));
  }
